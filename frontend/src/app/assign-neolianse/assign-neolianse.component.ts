...
@Component({
  selector: 'app-assign-neolianse',
  templateUrl: './assign-neolianse.component.html',
  styleUrls: ['./assign-neolianse.component.css']
})
export class AssignNeolianseComponent implements OnInit {
  searchTerm = '';
  ...
  assignUser(user: User): void {
    ...
    if (this.assignedUsers.some(assigned => assigned.id === user.id)) {
      this.showNotification('error', 'Cet utilisateur est déjà assigné au projet Neolianse');
      return;
    }

    // Assigner l'utilisateur au projet Neolianse
    this.assignedUsers.push(user);

    // Mettre à jour la liste des utilisateurs disponibles
    this.filterUsers();

    this.showNotification('success', `${user.firstName} ${user.lastName} a été assigné au projet Neolianse`);
  }
  ...
  unassignUser(user: User): void {
    ...
    if (index > -1) {
      ...
      this.showNotification('success', `${user.firstName} ${user.lastName} a été retiré du projet Neolianse`);
    }
  }
